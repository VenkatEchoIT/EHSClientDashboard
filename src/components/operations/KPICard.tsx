...
export function KPICard({ data }: KPICardProps) {
  return (
    <div className="flex min-w-[190px] flex-1 flex-col justify-between rounded-2xl border border-[var(--color-border-soft)] bg-[var(--color-surface)] px-5 py-4 shadow-[0_1px_2px_rgba(36,33,29,0.04)] transition-shadow hover:shadow-[0_2px_10px_rgba(36,33,29,0.06)]">
      ...
      {data.visual === "sparkline" && data.sparkline && (
        <div className="mt-3">
          <Sparkline data={data.sparkline} color={sparklineColorByTone[data.tone]} />
        </div>
      )}

      {data.visual === "bar" && data.barValue !== undefined && (
        <div className="mt-3 h-1.5 w-full overflow-hidden rounded-full bg-[var(--color-neutral-soft)]">
          <div
